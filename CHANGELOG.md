...
- **Corrected the install guidance in `README.md` and `skills/README.md`.** Three claims were wrong. The folder build does not work with Figma, because Figma does not read this repository. Figma custom skills and Figma skills for MCP are two different things that share a word, and the MCP kind installs into your coding agent rather than into Figma. Figma Make was listed among chat based builders that read a file from the project, which it does not. Adds the Community link, the upload steps, the slash command, that a skill stays available across all your files, and the seat and edit access requirements.
